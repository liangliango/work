    ...
    function refresh(id) {
        if (id !== 4){
            table.render({
                ...
                height: 'full-170',
                url: nginx_url + '/customer/findWayBillbyStateAndSendId/'+array[id]+'/'+customerId,
                datatype:"json",
                limit: 10,
                limits: [10],
                request: {
        ...
        if (id===4){
            table.render({
                elem: '#goodsBillTable' + (id + 1),
                height: 'full-170',
                url: nginx_url + '/customer/findAllWayBillBySendId/'+customerId,
                limit: 10,
                limits: [10],
                request: {
                    pageName: 'pageNum' //页码的参数名称，默认：page
