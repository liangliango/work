    ...
    element.on('tab(groupFilter)', function (data) {
        if (data.index === 1) {
            table.render({
                elem: '#groupTable',
                height: 'full-170',
                url: nginx_url + '/manger/findAllGroupByPage', //数据接口
                limit: 10,
                limits: [10],
                request: {
            ...
            table.on('tool(groupTool)', function (obj) { //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
                ...
                if (layEvent === 'del') { //删除
                    layer.confirm('真的删除么', function (index) {
                        ...
                        $.ajax({
                            type: "DELETE",
                            url: nginx_url + "/manger/deleteGroup/" + data.id,
                        ...
                        layer.msg('删除成功', {
                            ...
                            table.reload('groupTable', {
                                url: nginx_url + '/manger/findAllGroupByPage'
                            })
                        });
